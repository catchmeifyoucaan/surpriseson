...
struct IOSBridgeChatTransport: SurprisebotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SurprisebotChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SurprisebotChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SurprisebotChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SurprisebotChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SurprisebotChatAttachmentPayload]) async throws -> SurprisebotChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SurprisebotChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(SurprisebotChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.bridge.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(SurprisebotGatewayHealthOK.self, from: res))?.ok ?? true
    }
            ...
            let task = Task {
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? JSONDecoder().decode(SurprisebotGatewayHealthOK.self, from: data))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let json = evt.payloadJSON, let data = json.data(using: .utf8) else { break }
                        if let payload = try? JSONDecoder().decode(SurprisebotChatEventPayload.self, from: data) {
                            continuation.yield(.chat(payload))
                        }
                    case "agent":
                        guard let json = evt.payloadJSON, let data = json.data(using: .utf8) else { break }
                        if let payload = try? JSONDecoder().decode(SurprisebotAgentEventPayload.self, from: data) {

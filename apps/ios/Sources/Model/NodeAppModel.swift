...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = SurprisebotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = SurprisebotCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = SurprisebotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = SurprisebotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = SurprisebotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__surprisebot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: SurprisebotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for surprisebot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case SurprisebotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case SurprisebotCanvasCommand.present.rawValue,
            ...
            case SurprisebotCanvasA2UICommand.reset.rawValue,
            ...
            case SurprisebotCameraCommand.list.rawValue,
                 SurprisebotCameraCommand.snap.rawValue,
                 SurprisebotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case SurprisebotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: SurprisebotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SurprisebotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(SurprisebotLocationGetParams.self, from: req.paramsJSON)) ??
            SurprisebotLocationGetParams()
        ...
        let payload = SurprisebotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SurprisebotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(SurprisebotCanvasPresentParams.self, from: req.paramsJSON)) ??
                SurprisebotCanvasPresentParams()
        ...
        case SurprisebotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SurprisebotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(SurprisebotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SurprisebotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(SurprisebotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SurprisebotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(SurprisebotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SurprisebotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.surprisebotA2UI) return JSON.stringify({ ok: false, error: "missing surprisebotA2UI" });
              return JSON.stringify(globalThis.surprisebotA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case SurprisebotCanvasA2UICommand.push.rawValue, SurprisebotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == SurprisebotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(SurprisebotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try SurprisebotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(SurprisebotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try SurprisebotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.surprisebotA2UI) return JSON.stringify({ ok: false, error: "missing surprisebotA2UI" });
                const messages = \(messagesJSON);
                return JSON.stringify(globalThis.surprisebotA2UI.applyMessages(messages));
              } catch (e) {
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SurprisebotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SurprisebotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(SurprisebotCameraSnapParams.self, from: req.paramsJSON)) ??
                SurprisebotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SurprisebotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(SurprisebotCameraClipParams.self, from: req.paramsJSON)) ??
                SurprisebotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(SurprisebotScreenRecordParams.self, from: req.paramsJSON)) ??
            SurprisebotScreenRecordParams()
    ...
    private func locationMode() -> SurprisebotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return SurprisebotLocationMode(rawValue: raw) ?? .off
    }

...
private func printUsage() {
    print("""
    surprisebot-mac-wizard

    Usage:
      surprisebot-mac-wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
...
private func resolvedToken(opts: WizardCliOptions, config: GatewayConfig) -> String? {
    ...
    if let token = ProcessInfo.processInfo.environment["SURPRISEBOT_GATEWAY_TOKEN"], !token.isEmpty {
        return token
    }
...
private func resolvedPassword(opts: WizardCliOptions, config: GatewayConfig) -> String? {
    ...
    if let password = ProcessInfo.processInfo.environment["SURPRISEBOT_GATEWAY_PASSWORD"], !password.isEmpty {
        return password
    }
    if (config.mode ?? "local").lowercased() == "remote" {
...
actor GatewayWizardClient {
    ...
    private func sendConnect() async throws {
        ...
        let client: [String: AnyCodable] = [
            "id": AnyCodable("surprisebot-macos"),
            "displayName": AnyCodable(Host.current().localizedName ?? "Surprisebot macOS Wizard CLI"),

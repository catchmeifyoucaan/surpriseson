...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.surprisebot/surprisebot.json. Prefer SURPRISEBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.surprisebot/surprisebot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.surprisebot/surprisebot.json. Restart the gateway to apply."
        }

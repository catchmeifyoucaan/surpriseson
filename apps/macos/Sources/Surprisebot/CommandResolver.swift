...
enum CommandResolver {
    private static let projectRootDefaultsKey = "surprisebot.gatewayProjectRootPath"
    private static let helperName = "surprisebot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager.default.isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/surprisebot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/surprisebot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        let surprisebotPaths = self.surprisebotManagedPaths(home: home)
        if !surprisebotPaths.isEmpty {
            extras.insert(contentsOf: surprisebotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + surprisebotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func surprisebotManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".surprisebot")
    ...
    static func surprisebotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSurprisebotExecutable(projectRoot: URL? = nil) -> String? {
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/surprisebot.js").path
        return FileManager.default.isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnySurprisebotInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.surprisebotExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func surprisebotNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let surprisebotPath = self.projectSurprisebotExecutable(projectRoot: root) {
                return [surprisebotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "surprisebot", subcommand] + extraArgs
            }
            if let surprisebotPath = self.surprisebotExecutable(searchPaths: searchPaths) {
                return [surprisebotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            surprisebot entrypoint missing (looked for dist/index.js or bin/surprisebot.js); run pnpm build.
            """
    ...
    // Existing callers still refer to surprisebotCommand; keep it as node alias.
    static func surprisebotCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        searchPaths: [String]? = nil) -> [String]
    {
        self.surprisebotNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real surprisebot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/surprisebot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v surprisebot >/dev/null 2>&1; then
          CLI="$(command -v surprisebot)"
          surprisebot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/surprisebot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/surprisebot.js"
            node "$PRJ/bin/surprisebot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent surprisebot"
          pnpm --silent surprisebot \(quotedArgs);
        else
          echo "surprisebot CLI missing on remote host"; exit 127;
        fi
        """
    ...
    static func connectionSettings(defaults: UserDefaults = .standard) -> RemoteSettings {
        ...
        if let modeRaw {
            ...
            let seen = defaults.bool(forKey: "surprisebot.onboardingSeen")

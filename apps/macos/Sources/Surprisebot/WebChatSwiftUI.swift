...
private let webChatSwiftLogger = Logger(subsystem: "com.surprisebot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SurprisebotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SurprisebotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SurprisebotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SurprisebotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                        SurprisebotGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                        SurprisebotChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                        SurprisebotAgentEventPayload.self,
                        from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SurprisebotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SurprisebotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SurprisebotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SurprisebotChatView(
                                            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SurprisebotChatView>) -> NSViewController
    {

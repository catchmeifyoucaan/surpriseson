...
private let gatewayConnectionLogger = Logger(subsystem: "com.surprisebot", category: "gateway.connection")
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(SurprisebotGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func chatHistory(
        sessionKey: String,
        limit: Int? = nil,
        timeoutMs: Int? = nil) async throws -> SurprisebotChatHistoryPayload
    {
    ...
    func chatSend(
        ...
        attachments: [SurprisebotChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> SurprisebotChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        var params: [String: AnyCodable] = [

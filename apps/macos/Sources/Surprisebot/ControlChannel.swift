...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "com.surprisebot", category: "control")
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            return
                "Gateway rejected token; set gateway.auth.token (or SURPRISEBOT_GATEWAY_TOKEN) " +
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("surprisebot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("surprisebot.control.agent")
}

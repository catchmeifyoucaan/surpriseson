...
struct SessionMenuPreviewView: View {
    private static let logger = Logger(subsystem: "com.surprisebot", category: "SessionPreview")
    ...
    private static func previewItems(
        from payload: SurprisebotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let raw: [SurprisebotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SurprisebotKit.AnyCodable]) -> [SurprisebotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SurprisebotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SurprisebotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SurprisebotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SurprisebotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SurprisebotChatMessage) -> String? {

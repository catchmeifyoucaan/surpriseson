...
struct ConfigSettings: View {
    ...
    private static let browserAttachOnlyHelp =
        "When enabled, the browser server will only connect if the surprisebot browser is already running."
    private static let browserProfileNote =
        "Surprisebot uses a separate Chrome profile and ports (default 18791/18792) "
    ...
    // surprisebot browser settings (stored in ~/.surprisebot/surprisebot.json under "browser")
    ...
    // Talk mode settings (stored in ~/.surprisebot/surprisebot.json under "talk")
...
extension ConfigSettings {
    ...
    private var header: some View {
        Text("Surprisebot CLI config")
            .font(.title3.weight(.semibold))
        Text(self.isNixMode
                ? "This tab is read-only in Nix mode. Edit config via Nix and rebuild."
                : "Edit ~/.surprisebot/surprisebot.json (agent / session / routing / messages).")
            .font(.callout)
    ...
    private var anthropicAuthHelpText: String {
        "Determined from Surprisebot OAuth token file (~/.surprisebot/credentials/oauth.json) " +

...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "com.surprisebot", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Surprisebot run already has an SSH listener on the expected port (common after restarts),

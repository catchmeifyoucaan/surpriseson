...
enum SurprisebotLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SurprisebotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SurprisebotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SurprisebotLogging.bootstrapIfNeeded()
        let label = SurprisebotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SurprisebotOSLogHandler: LogHandler {
...
struct SurprisebotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SurprisebotLogging.parseLabel(self.label)

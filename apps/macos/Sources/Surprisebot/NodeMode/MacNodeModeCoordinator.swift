...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "com.surprisebot", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: SurprisebotNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [SurprisebotCapability.canvas.rawValue, SurprisebotCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(SurprisebotCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if SurprisebotLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(SurprisebotCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            SurprisebotSystemCommand.run.rawValue,
            SurprisebotSystemCommand.notify.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(SurprisebotCapability.camera.rawValue) {
        ...
        if capsSet.contains(SurprisebotCapability.location.rawValue) {
    ...
    static func loopbackBridgePort() -> UInt16? {
        if let raw = ProcessInfo.processInfo.environment["SURPRISEBOT_BRIDGE_PORT"],
    ...
    static func remoteBridgePort() -> Int {
        ...
        let base =
            SurprisebotConfigFile.remoteGatewayPort(matchingHost: sshHost) ??
    ...
    static func probeEndpoint(_ endpoint: NWEndpoint, timeoutSeconds: Double) async -> Bool {
        ...
        connection.start(queue: DispatchQueue(label: "com.surprisebot.macos.bridge-loopback-probe"))
        do {
    ...
    private static func discoverBridgeEndpoint(timeoutSeconds: Double) async -> NWEndpoint? {
        ...
        return await withCheckedContinuation { cont in
            ...
            for domain in SurprisebotBonjour.bridgeServiceDomains {
                let browser = NWBrowser(
                    for: .bonjour(type: SurprisebotBonjour.bridgeServiceType, domain: domain),
                    using: params)
                ...
                browser.start(queue: DispatchQueue(label: "com.surprisebot.macos.bridge-discovery.\(domain)"))
            }
...
enum MacNodeTokenStore {
    private static let suiteName = "com.surprisebot.shared"

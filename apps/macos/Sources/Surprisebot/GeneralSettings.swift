...
struct GeneralSettings: View {
    ...
    @AppStorage(locationModeKey) private var locationModeRaw: String = SurprisebotLocationMode.off.rawValue
    ...
    @State private var lastLocationModeRaw: String = SurprisebotLocationMode.off.rawValue
    ...
    var body: some View {
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 18) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    SettingsToggleRow(
                        title: "Surprisebot active",
                        subtitle: "Pause to stop the Surprisebot gateway; no messages will be processed.",
                    ...
                    SettingsToggleRow(
                        ...
                        subtitle: "Automatically start Surprisebot after you sign in.",
                        binding: self.$state.launchAtLogin)

                    SettingsToggleRow(
                        title: "Show Dock icon",
                        subtitle: "Keep Surprisebot visible in the Dock instead of menu-bar-only mode.",
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Picker("", selection: self.$locationModeRaw) {
                            Text("Off").tag(SurprisebotLocationMode.off.rawValue)
                            Text("While Using").tag(SurprisebotLocationMode.whileUsing.rawValue)
                            Text("Always").tag(SurprisebotLocationMode.always.rawValue)
                        }
                ...
                HStack {
                    Spacer()
                    Button("Quit Surprisebot") { NSApp.terminate(nil) }
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = SurprisebotLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: SurprisebotLocationMode {
        SurprisebotLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: SurprisebotLocationMode) async -> Bool {
            ...
            DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    LabeledContent("CLI path") {
                        TextField("/Applications/Surprisebot.app/.../surprisebot", text: self.$state.remoteCliPath)
                            .textFieldStyle(.roundedBorder)
                            .frame(width: 280)
                    }
                }
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for surprisebot logs in /tmp/surprisebot/.
    ...
    private func applyDiscoveredGateway(_ gateway: GatewayDiscoveryModel.DiscoveredGateway) {
        ...
        SurprisebotConfigFile.setRemoteGatewayUrl(host: host, port: gateway.gatewayPort)
    }
}
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/surprisebot"
        state.remoteCliPath = "/tmp/surprisebot"
        ...
        view.cliInstallLocation = "/usr/local/bin/surprisebot"

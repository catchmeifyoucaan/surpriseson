...
final class HealthStore {
    ...
    private static let logger = Logger(subsystem: "com.surprisebot", category: "health")
    ...
    var summaryLine: String {
        ...
        if link.summary.linked != true {
            if let fallback = self.resolveFallbackChannel(snap, excluding: link.id) {
                ...
                return "\(fallbackLabel) \(fallbackState) · Not linked — run surprisebot login"
            }
            return "Not linked — run surprisebot login"
        }
        ...
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                ...
                return "The gateway control port (127.0.0.1:\(port)) isn’t listening — " +
                    "restart Surprisebot to bring it back."
            }

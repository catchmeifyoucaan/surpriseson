...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "com.surprisebot", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["SURPRISEBOT_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = SurprisebotConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.surprisebotExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expected?.description,
                    message: "surprisebot CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(version: Semver?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "surprisebot@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "surprisebot@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "surprisebot@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "surprisebot@\(target)"])
            }

        statusHandler("Installing surprisebot@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed surprisebot@\(target)")
        } else {

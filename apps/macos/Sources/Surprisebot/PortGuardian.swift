...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "com.surprisebot", category: "portguard")
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("Surprisebot", isDirectory: true)
    }()
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "surprisebot", "tsx", "pnpm", "bun"]
    ...
    private func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .remote:
            ...
            return false
        case .local:
            if !cmd.contains("surprisebot") { return false }
            if full.contains("gateway-daemon") { return true }
            // If args are unavailable, treat a surprisebot listener as expected.
            return full == cmd
        case .unconfigured:
            return false
        }

...
private let deepLinkLogger = Logger(subsystem: "com.surprisebot", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Surprisebot is paused", message: "Unpause Surprisebot to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Surprisebot agent?", message: body) else { return }
        }

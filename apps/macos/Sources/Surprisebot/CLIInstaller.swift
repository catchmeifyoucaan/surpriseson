...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("surprisebot").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing surprisebot CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed surprisebot \($0)." } ?? "Installed surprisebot."
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://surprisebot.bot/install-cli.sh | \

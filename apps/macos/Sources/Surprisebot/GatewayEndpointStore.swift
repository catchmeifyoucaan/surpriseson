...
actor GatewayEndpointStore {
    ...
    {
        let raw = env["SURPRISEBOT_GATEWAY_PASSWORD"] ?? ""
    ...
    private static func resolveGatewayToken(
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        let raw = env["SURPRISEBOT_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
            return trimmed
        }
    ...
    private let logger = Logger(subsystem: "com.surprisebot", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "surprisebot.onboardingSeen")
            initialMode = seen ? .local : .unconfigured
        }

        let port = deps.localPort()
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: SurprisebotConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
    ...
    {
        if let envBind = env["SURPRISEBOT_GATEWAY_BIND"] {

...
final class GatewayProcessManager {
    ...
    private let logger = Logger(subsystem: "com.surprisebot", category: "gateway.process")
    ...
    private func describeAttachFailure(_ error: Error, port: Int, instance: PortGuardian.Descriptor?) -> String {
        ...
        if self.isGatewayAuthFailure(error) {
            return """
            Gateway on port \(port) rejected auth. Set gateway.auth.token (or SURPRISEBOT_GATEWAY_TOKEN) \

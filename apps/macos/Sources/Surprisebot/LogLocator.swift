...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SURPRISEBOT_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/surprisebot")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("surprisebot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("surprisebot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/surprisebot/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("surprisebot") && $0.pathExtension == "log" }
            .max { lhs, rhs in

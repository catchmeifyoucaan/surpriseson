...
private typealias ProtoAnyCodable = SurprisebotProtocol.AnyCodable

actor GatewayChannelActor {
    private let logger = Logger(subsystem: "com.surprisebot", category: "gateway")
    ...
    private func sendConnect() async throws {
        ...
        let clientId = "surprisebot-macos"
    ...
    func request(
        method: String,
        params: [String: SurprisebotProtocol.AnyCodable]?,
        timeoutMs: Double? = nil) async throws -> Data
    {
        ...
        if res.ok == false {
            ...
            let details: [String: SurprisebotProtocol.AnyCodable] = (res.error ?? [:]).reduce(into: [:]) { acc, pair in
                acc[pair.key] = SurprisebotProtocol.AnyCodable(pair.value.value)
            }

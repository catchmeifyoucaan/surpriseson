...
enum GatewayLaunchAgentManager {
    private static let logger = Logger(subsystem: "com.surprisebot", category: "gateway.launchd")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]
    private static let legacyGatewayLaunchdLabel = "com.steipete.surprisebot.gateway"
    private static let disableLaunchAgentMarker = ".surprisebot/disable-launchagent"
        ...
        bind: String) -> Result<[String], GatewayProgramArgumentsError>
    {
        ...
        #if DEBUG
        if let localBin = CommandResolver.projectSurprisebotExecutable(projectRoot: projectRoot) {
        ...
        if let gatewayBin = CommandResolver.surprisebotExecutable(searchPaths: searchPaths) {
        ...
        return .failure(.message("surprisebot CLI not found in PATH; install the CLI."))
    }
    ...
    private static func writePlist(programArguments: [String]) {
        ...
        if let token {
            ...
            envEntries += """
                <key>SURPRISEBOT_GATEWAY_TOKEN</key>
        ...
        if let password {
            ...
            envEntries += """
                <key>SURPRISEBOT_GATEWAY_PASSWORD</key>
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["SURPRISEBOT_GATEWAY_BIND"] {
    ...
    private static func preferredGatewayToken() -> String? {
        let raw = ProcessInfo.processInfo.environment["SURPRISEBOT_GATEWAY_TOKEN"] ?? ""
    ...
    private static func preferredGatewayPassword() -> String? {
        ...
        let raw = ProcessInfo.processInfo.environment["SURPRISEBOT_GATEWAY_PASSWORD"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
            return trimmed
        }

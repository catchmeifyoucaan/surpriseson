...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SurprisebotOAuthStore.AnthropicOAuthStatus = SurprisebotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager.default.fileExists(atPath: SurprisebotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = SurprisebotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SurprisebotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try SurprisebotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Surprisebot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SurprisebotOAuthStore.AnthropicOAuthStatus,

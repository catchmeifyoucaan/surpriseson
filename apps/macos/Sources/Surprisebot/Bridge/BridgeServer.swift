...
actor BridgeServer {
    ...
    private let logger = Logger(subsystem: "com.surprisebot", category: "bridge")
    ...
    func start() async {
        ...
        do {
            ...
            listener.start(queue: DispatchQueue(label: "com.surprisebot.bridge"))
    ...
    private func handleRequest(nodeId: String, req: BridgeRPCRequest) async -> BridgeRPCResponse {
        ...
        let params: [String: SurprisebotProtocol.AnyCodable]?
        if let json = req.paramsJSON?.trimmingCharacters(in: .whitespacesAndNewlines), !json.isEmpty {
            ...
            do {
                params = try JSONDecoder().decode([String: SurprisebotProtocol.AnyCodable].self, from: data)
            } catch {
                return BridgeRPCResponse(
                    id: req.id,
                    ok: false,
    ...
    private func beaconPresence(nodeId: String, reason: String) async {
        ...
        var params: [String: SurprisebotProtocol.AnyCodable] = [
            "text": SurprisebotProtocol.AnyCodable(summary),
            "instanceId": SurprisebotProtocol.AnyCodable(nodeId),
            "host": SurprisebotProtocol.AnyCodable(host),
            "mode": SurprisebotProtocol.AnyCodable("node"),
            "reason": SurprisebotProtocol.AnyCodable(reason),
            "tags": SurprisebotProtocol.AnyCodable(tags),
        ]
        if let ip { params["ip"] = SurprisebotProtocol.AnyCodable(ip) }
        if let version { params["version"] = SurprisebotProtocol.AnyCodable(version) }
    ...
    private static func defaultStoreURL() throws -> URL {
        ...
        return base
            .appendingPathComponent("Surprisebot", isDirectory: true)
...
enum BridgePairingApprover {
    static func approve(request: BridgePairRequest, isRepair: Bool) async -> Bool {
        await withCheckedContinuation { cont in
            ...
            let alert = NSAlert()
            alert.messageText = isRepair ? "Re-pair Surprisebot Node?" : "Pair Surprisebot Node?"

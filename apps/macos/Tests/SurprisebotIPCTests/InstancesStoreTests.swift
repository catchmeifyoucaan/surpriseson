...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SurprisebotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SurprisebotProtocol.AnyCodable] = [
            "presence": .init([SurprisebotProtocol.AnyCodable(entry)]),
        ]
        let payload = SurprisebotProtocol.AnyCodable(payloadMap)

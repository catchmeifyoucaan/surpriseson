...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func gatewayEntrypointPrefersDistOverBin() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/surprisebot.js")
    ...
    @Test func logLocatorPicksNewestLogFile() throws {
        ...
        let dir = URL(fileURLWithPath: "/tmp/surprisebot", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("surprisebot-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("surprisebot-new-\(UUID().uuidString).log")

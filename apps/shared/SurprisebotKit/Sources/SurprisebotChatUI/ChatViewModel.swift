...
private let chatUILogger = Logger(subsystem: "com.surprisebot", category: "SurprisebotChatUI")

@MainActor
@Observable
public final class SurprisebotChatViewModel {
    public private(set) var messages: [SurprisebotChatMessage] = []
    ...
    public var attachments: [SurprisebotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SurprisebotChatPendingToolCall] = []
    public private(set) var sessions: [SurprisebotChatSessionEntry] = []
    private let transport: any SurprisebotChatTransport
    ...
    private var pendingToolCallsById: [String: SurprisebotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SurprisebotChatTransport) {
    ...
    public var sessionChoices: [SurprisebotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [SurprisebotChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [SurprisebotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SurprisebotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SurprisebotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SurprisebotChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [SurprisebotChatMessage]) -> [SurprisebotChatMessage] {
        var result: [SurprisebotChatMessage] = []
    ...
    private static func dedupeKey(for message: SurprisebotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SurprisebotChatMessageContent] = [
            SurprisebotChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> SurprisebotChatAttachmentPayload in
            SurprisebotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SurprisebotChatSessionEntry {
        SurprisebotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SurprisebotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SurprisebotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SurprisebotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SurprisebotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SurprisebotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)

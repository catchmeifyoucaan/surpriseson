...
struct ChatMessageBubble: View {
    let message: SurprisebotChatMessage
    let style: SurprisebotChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SurprisebotChatTheme.userText : SurprisebotChatTheme.assistantText
                ...
                if !split.images.isEmpty {
                    ...
                    { (item: ChatMarkdownSplitter.InlineImage) in
                        if let img = item.image {
                            SurprisebotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFit()
                                .frame(maxHeight: 260)
    ...
    private var inlineAttachments: [SurprisebotChatMessageContent] {
    ...
    private var toolCalls: [SurprisebotChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SurprisebotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SurprisebotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SurprisebotChatTheme.onboardingAssistantBubble
        }
        return SurprisebotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SurprisebotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SurprisebotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SurprisebotChatTheme.userText : SurprisebotChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SurprisebotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SurprisebotChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SurprisebotChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SurprisebotChatPendingToolCall]
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownBody(text: segment.text, textColor: SurprisebotChatTheme.assistantText, font: font)
            }
        }
    }
}

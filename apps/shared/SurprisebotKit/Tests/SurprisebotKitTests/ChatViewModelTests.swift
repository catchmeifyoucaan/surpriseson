...
import Foundation
import Testing
@testable import SurprisebotChatUI
...
private final class TestChatTransport: @unchecked Sendable, SurprisebotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SurprisebotChatHistoryPayload]
    private let sessionsResponses: [SurprisebotChatSessionsListResponse]

    private let stream: AsyncStream<SurprisebotChatTransportEvent>
    private let continuation: AsyncStream<SurprisebotChatTransportEvent>.Continuation

    init(
        historyResponses: [SurprisebotChatHistoryPayload],
        sessionsResponses: [SurprisebotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SurprisebotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SurprisebotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SurprisebotChatHistoryPayload(
        ...
        attachments _: [SurprisebotChatAttachmentPayload]) async throws -> SurprisebotChatSendResponse
    {
        ...
        return SurprisebotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SurprisebotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SurprisebotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SurprisebotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SurprisebotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SurprisebotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SurprisebotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SurprisebotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SurprisebotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SurprisebotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SurprisebotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }

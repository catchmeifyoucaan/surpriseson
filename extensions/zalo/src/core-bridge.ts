...
function resolveSurprisebotRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.SURPRISEBOT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    const found = findPackageRoot(start, "surprisebot");
    if (found) {
      coreRootCache = found;
      return found;
    }
  ...
  throw new Error(
    "Unable to resolve Surprisebot root. Set SURPRISEBOT_ROOT to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveSurprisebotRoot();

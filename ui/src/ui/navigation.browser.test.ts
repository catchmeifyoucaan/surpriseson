...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/surprisebot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/surprisebot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/surprisebot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SURPRISEBOT_CONTROL_UI_BASE_PATH__ = "/surprisebot";
    const app = mountApp("/surprisebot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/surprisebot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/surprisebot/sessions");
  });

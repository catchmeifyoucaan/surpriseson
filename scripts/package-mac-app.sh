...
# Build and bundle Surprisebot into a minimal .app we can open.
# Outputs to dist/Surprisebot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Surprisebot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Surprisebot"
BUNDLE_ID="${BUNDLE_ID:-com.surprisebot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/surprisebot/surprisebot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Surprisebot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SurprisebotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SurprisebotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Surprisebot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Surprisebot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Surprisebot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Surprisebot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Surprisebot/Resources/Surprisebot.icns" "$APP_ROOT/Contents/Resources/Surprisebot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Surprisebot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"

echo "📦 Copying SurprisebotKit resources"
SURPRISEBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SurprisebotKit_SurprisebotKit.bundle"
if [ -d "$SURPRISEBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SurprisebotKit_SurprisebotKit.bundle"
  cp -R "$SURPRISEBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SurprisebotKit_SurprisebotKit.bundle"
else
  echo "WARN: SurprisebotKit resource bundle not found at $SURPRISEBOTKIT_BUNDLE (continuing)" >&2
fi

echo "⏹  Stopping any running Surprisebot"
killall -q Surprisebot 2>/dev/null || true

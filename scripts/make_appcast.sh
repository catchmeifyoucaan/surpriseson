...
ZIP=${1:?"Usage: $0 Surprisebot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/surprisebot/surprisebot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Surprisebot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/surprisebot/surprisebot/releases/download/v${VERSION}/"}

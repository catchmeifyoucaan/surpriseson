...
BASE_IMAGE="${BASE_IMAGE:-surprisebot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-surprisebot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=surprisebot.sandbox=1)
NOTE

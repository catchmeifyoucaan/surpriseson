...
# Surprisebot Auth Widget for Termux
...
SERVER="${SURPRISEBOT_SERVER:-l36}"

# Check auth status
termux-toast "Checking Surprisebot auth..."

STATUS=$(ssh "$SERVER" '$HOME/surprisebot/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/surprisebot/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|SURPRISEBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/surprisebot/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|SURPRISEBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Surprisebot won't work until re-auth"
                ;;
        esac
        ;;

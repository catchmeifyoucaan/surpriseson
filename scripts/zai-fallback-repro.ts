...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "surprisebot-zai-fallback-"),
  );
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "surprisebot.json");
  ...
  const sessionId =
    process.env.SURPRISEBOT_ZAI_FALLBACK_SESSION_ID ?? randomUUID();

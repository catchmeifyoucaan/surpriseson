...
IMAGE_NAME="surprisebot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/surprisebot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/surprisebot-bin/systemctl

  cat > /tmp/surprisebot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
	  chmod +x /tmp/surprisebot-bin/loginctl
	  ...
	  npm_bin="/tmp/npm-prefix/bin/surprisebot"
	  npm_entry="/tmp/npm-prefix/lib/node_modules/surprisebot/dist/entry.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/surprisebot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/surprisebot-gateway.service"
    if [ ! -f "$unit_path" ]; then

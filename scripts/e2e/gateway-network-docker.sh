...
IMAGE_NAME="surprisebot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="surprisebot-net-e2e-$$"
GW_NAME="surprisebot-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SURPRISEBOT_GATEWAY_TOKEN=$TOKEN" \
	  -e "SURPRISEBOT_SKIP_CHANNELS=1" \
	  -e "SURPRISEBOT_SKIP_GMAIL_WATCHER=1" \
	  -e "SURPRISEBOT_SKIP_CRON=1" \
	  -e "SURPRISEBOT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \

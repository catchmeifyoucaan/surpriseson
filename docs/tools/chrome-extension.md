---
summary: "Chrome extension: let Surprisebot drive your existing Chrome tab"
read_when:
...
The Surprisebot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate surprisebot-managed Chrome profile.
...
Surprisebot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Surprisebot release (npm package) as static files. There is no separate “build” step.

After upgrading Surprisebot:
- Re-run `surprisebot browser extension install` to refresh the installed files under your Surprisebot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Surprisebot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Surprisebot to control.
...
- `ON`: attached; Surprisebot can drive that tab.
- `…`: connecting to the local relay.
- `!`: relay not reachable (most common: browser relay server isn’t running on this machine).

If you see `!`:
- Make sure the Gateway is running locally (default setup), or run `surprisebot browser serve` on this machine (remote gateway setup).
- Open the extension Options page; it shows whether the relay is reachable.

## Do I need `surprisebot browser serve`?
...
you typically **do not** need `surprisebot browser serve`.

The Gateway’s built-in browser control server will start on `http://127.0.0.1:18791/` and Surprisebot will
auto-start the local relay server on `http://127.0.0.1:18792/`.

### Remote Gateway (Gateway runs elsewhere) — **yes**

If your Gateway runs on another machine, run `surprisebot browser serve` on the machine that runs Chrome
...
Debugging: `surprisebot sandbox explain`
...
`surprisebot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `surprisebot browser extension install` first to copy the extension to a stable location under your Surprisebot state directory.
...
- **This is not isolated** like the dedicated surprisebot-managed profile.

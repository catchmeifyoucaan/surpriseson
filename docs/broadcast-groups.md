...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Surprisebot would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Surprisebot would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/surprisebot-alfred/
...
Workspace: /Users/pascal/surprisebot-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.surprisebot/logs/gateway.log | grep broadcast
```

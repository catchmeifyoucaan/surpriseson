---
summary: "First-run onboarding flow for Surprisebot (macOS app)"
read_when:
...
- Writes credentials to `~/.surprisebot/credentials/oauth.json`
...
The app can install the global `surprisebot` CLI via npm/pnpm so terminal
...
On the first agent run, Surprisebot bootstraps a workspace (default `~/surprisebot`):
...
```bash
surprisebot hooks gmail setup --account you@gmail.com
```
...
- `~/.surprisebot/credentials/oauth.json`

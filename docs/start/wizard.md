...
The onboarding wizard is the **recommended** way to set up Surprisebot on macOS,
...
1) **Existing config detection**
   - If `~/.surprisebot/surprisebot.json` exists, choose **Keep / Modify / Reset**.
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `surprisebot doctor` before continuing.
...
2) **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.surprisebot/.env` so launchd can read it.
  ...
  - OAuth credentials live in `~/.surprisebot/credentials/oauth.json`; auth profiles live in `~/.surprisebot/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
   - More detail: [/concepts/oauth](/concepts/oauth)

3) **Workspace**
   - Default `~/surprisebot` (configurable).
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `surprisebot pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `surprisebot health`.
   - Tip: `surprisebot status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `surprisebot agents add <name>` to create a separate agent with its own workspace,
...
Notes:
- Default workspaces follow `~/surprisebot-<agentId>`.
...
```bash
surprisebot onboard --non-interactive \
  --mode local \
  --auth-choice apiKey \
...
- Stores it under `~/.surprisebot/tools/signal-cli/<version>/`.
...
Typical fields in `~/.surprisebot/surprisebot.json`:
...
`surprisebot agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.surprisebot/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.surprisebot/agents/<agentId>/sessions/`.

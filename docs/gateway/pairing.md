...
surprisebot nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.surprisebot`):

- `~/.surprisebot/nodes/paired.json`
- `~/.surprisebot/nodes/pending.json`

If you override `SURPRISEBOT_STATE_DIR`, the `nodes/` folder moves with it.

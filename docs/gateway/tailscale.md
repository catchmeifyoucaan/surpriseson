...
Surprisebot can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Surprisebot requires a shared password.
...
- `token` (default when `SURPRISEBOT_GATEWAY_TOKEN` is set)
- `password` (shared secret via `SURPRISEBOT_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. Surprisebot only
...
Prefer `SURPRISEBOT_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
surprisebot gateway --tailscale serve
surprisebot gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Surprisebot to undo `tailscale serve`

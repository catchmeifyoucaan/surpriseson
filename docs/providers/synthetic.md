---
summary: "Use Synthetic's Anthropic-compatible API in Surprisebot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Surprisebot registers it as the
...
```bash
surprisebot onboard --auth-choice synthetic-api-key
```
...
Note: Surprisebot's Anthropic client appends `/v1` to the base URL, so use

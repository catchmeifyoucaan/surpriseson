...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/surprisebot`)
...
If the page fails to load, start the Gateway first: `surprisebot gateway`.
...
- Config: view/edit `~/.surprisebot/surprisebot.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Surprisebot
...
```bash
surprisebot gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
```bash
SURPRISEBOT_CONTROL_UI_BASE_PATH=/surprisebot/ pnpm ui:build
```

...
- CLI: `surprisebot status --usage` prints a full per-provider breakdown.
- CLI: `surprisebot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

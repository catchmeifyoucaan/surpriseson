---
summary: "Run Surprisebot Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: Surprisebot Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install Surprisebot  
2) Run `surprisebot onboard --install-daemon`  
...
```bash
ssh exe.dev new --name=surprisebot
```

Then connect:

```bash
ssh surprisebot.exe.xyz
```

Tip: keep this VM **stateful**. Surprisebot stores state under `~/.surprisebot/` and `~/surprisebot/`.
...
It can set up:
- `~/surprisebot` workspace bootstrap
...
```bash
ssh -N -L 18789:127.0.0.1:18789 surprisebot.exe.xyz
```
...
```bash
export SURPRISEBOT_GATEWAY_TOKEN="$(openssl rand -hex 32)"
surprisebot gateway --bind lan --port 8080 --token "$SURPRISEBOT_GATEWAY_TOKEN"
```

For daemon runs, persist it in `~/.surprisebot/surprisebot.json`:
...
Notes:
- Non-loopback binds require `gateway.auth.token` (or `SURPRISEBOT_GATEWAY_TOKEN`).
- `gateway.remote.token` is only for remote CLI calls; it does not enable local auth.

Then point exe.dev’s proxy at `8080` (or whatever port you chose) and open your VM’s HTTPS URL:

```bash
ssh exe.dev share port surprisebot 8080
```

Open:
- `https://surprisebot.exe.xyz/`
...
On Linux, Surprisebot uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status surprisebot-gateway[-<profile>].service
```

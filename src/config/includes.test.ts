...
const ETC_SURPRISEBOT_DIR = path.join(ROOT_DIR, "etc", "surprisebot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "surprisebot.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSurprisebotPath(...parts: string[]) {
  return path.join(ETC_SURPRISEBOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcSurprisebotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "surprisebot.json"))).toEqual({
      shared: true,
    });
  });
});

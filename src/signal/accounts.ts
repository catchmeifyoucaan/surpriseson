...
export function listSignalAccountIds(cfg: SurprisebotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: SurprisebotConfig): string {
...
function mergeSignalAccountConfig(cfg: SurprisebotConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: SurprisebotConfig): ResolvedSignalAccount[] {

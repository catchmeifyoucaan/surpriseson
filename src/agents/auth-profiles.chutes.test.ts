...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) delete process.env.SURPRISEBOT_STATE_DIR;
    else process.env.SURPRISEBOT_STATE_DIR = previousStateDir;
    if (previousAgentDir === undefined) delete process.env.SURPRISEBOT_AGENT_DIR;
    else process.env.SURPRISEBOT_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-chutes-"));
    process.env.SURPRISEBOT_STATE_DIR = tempDir;
    process.env.SURPRISEBOT_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");

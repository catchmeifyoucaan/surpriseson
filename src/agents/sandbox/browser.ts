...
import { DEFAULT_SURPRISEBOT_BROWSER_COLOR } from "../../browser/constants.js";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    cdpHost,
    cdpIsLoopback: true,
    color: DEFAULT_SURPRISEBOT_BROWSER_COLOR,
    ...
    profiles: {
      surprisebot: { cdpPort: params.cdpPort, color: DEFAULT_SURPRISEBOT_BROWSER_COLOR },
    },
  };
}
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "surprisebot.sandboxBrowser": "1" },
    });
    ...
    args.push("-e", `SURPRISEBOT_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `SURPRISEBOT_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `SURPRISEBOT_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `SURPRISEBOT_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `SURPRISEBOT_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing ? resolveProfile(existing.bridge.state.resolved, "surprisebot") : null;
  const shouldReuse =

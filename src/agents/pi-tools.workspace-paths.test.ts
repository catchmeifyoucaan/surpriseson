...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("surprisebot-ws-", async (workspaceDir) => {
      await withTempDir("surprisebot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createSurprisebotCodingTools({ workspaceDir });
    ...
    await withTempDir("surprisebot-ws-", async (workspaceDir) => {
      await withTempDir("surprisebot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello surprisebot");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("surprisebot-ws-", async (workspaceDir) => {
      await withTempDir("surprisebot-override-", async (overrideDir) => {
        const tools = createSurprisebotCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("surprisebot-sandbox-", async (sandboxDir) => {
      await withTempDir("surprisebot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "surprisebot-sbx-test",
        ...
        const tools = createSurprisebotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");

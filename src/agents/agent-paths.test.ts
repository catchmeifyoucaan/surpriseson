...
describe("resolveSurprisebotAgentDir", () => {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-agent-"));
    process.env.SURPRISEBOT_STATE_DIR = tempStateDir;
    delete process.env.SURPRISEBOT_AGENT_DIR;
    delete process.env.PI_CODING_AGENT_DIR;

    const resolved = resolveSurprisebotAgentDir();

    expect(resolved).toBe(path.join(tempStateDir, "agents", "main", "agent"));
  });

  it("honors SURPRISEBOT_AGENT_DIR overrides", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-agent-"));
    const override = path.join(tempStateDir, "agent");
    process.env.SURPRISEBOT_AGENT_DIR = override;
    delete process.env.PI_CODING_AGENT_DIR;

...
import type { SurprisebotConfig, SkillConfig } from "../../config/config.js";
...
export function resolveConfigPath(config: SurprisebotConfig | undefined, pathStr: string) {
...
export function isConfigPathTruthy(config: SurprisebotConfig | undefined, pathStr: string): boolean {
...
function isBundledSkill(entry: SkillEntry): boolean {
  return entry.skill.source === "surprisebot-bundled";
}

export function resolveBundledAllowlist(config?: SurprisebotConfig): string[] | undefined {
...
export function shouldIncludeSkill(params: {
  ...
  const osList = entry.surprisebot?.os ?? [];
  ...
  if (entry.surprisebot?.always === true) {
    return true;
  }

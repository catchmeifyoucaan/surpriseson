...
describe("external CLI credential sync", () => {
  it("does not overwrite API keys when syncing external CLI creds", async () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "surprisebot-no-overwrite-"));
    try {
      await withTempHome(
        async (tempHome) => {
  ...
  it("prefers oauth over token even if token has later expiry (oauth enables auto-refresh)", async () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "surprisebot-cli-oauth-preferred-"));
    try {
      await withTempHome(
        async (tempHome) => {
          const claudeDir = path.join(tempHome, ".claude");

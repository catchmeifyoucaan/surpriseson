...
describe("external CLI credential sync", () => {
  it("does not overwrite fresher store oauth with older CLI oauth", async () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "surprisebot-cli-oauth-no-downgrade-"));
    try {
      await withTempHome(
        async (tempHome) => {
  ...
  it("does not downgrade store oauth to token when CLI lacks refresh token", async () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "surprisebot-cli-no-downgrade-oauth-"));
    try {
      await withTempHome(
        async (tempHome) => {
          const claudeDir = path.join(tempHome, ".claude");

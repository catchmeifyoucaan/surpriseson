...
import { __testing, createSurprisebotCodingTools } from "./pi-tools.js";

describe("createSurprisebotCodingTools", () => {
  ...
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from surprisebot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "surprisebot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "surprisebot-workspace"),
    ...
    const tools = createSurprisebotCodingTools({ sandbox });

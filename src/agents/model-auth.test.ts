...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-oauth-"));

    try {
      process.env.SURPRISEBOT_STATE_DIR = tempDir;
      process.env.SURPRISEBOT_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.SURPRISEBOT_AGENT_DIR, {
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-auth-"));
  ...
  it("rotates comma-separated env keys", async () => {
    const previousGemini = process.env.GEMINI_API_KEY;

    try {
      process.env.GEMINI_API_KEY = "gemini-key-1, gemini-key-2";
      vi.resetModules();
      const { resolveEnvApiKey } = await import("./model-auth.js");

      const first = resolveEnvApiKey("google");
      const second = resolveEnvApiKey("google");

      expect(first?.apiKey).toBe("gemini-key-1");
      expect(second?.apiKey).toBe("gemini-key-2");
    } finally {

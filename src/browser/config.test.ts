...
describe("browser config", () => {
  it("defaults to enabled with loopback control url and lobster-orange color", () => {
    ...
    const surprisebot = resolveProfile(resolved, "surprisebot");
    expect(surprisebot?.driver).toBe("surprisebot");
    expect(surprisebot?.cdpPort).toBe(18800);
    expect(surprisebot?.cdpUrl).toBe("http://127.0.0.1:18800");
  });

  it("derives default ports from SURPRISEBOT_GATEWAY_PORT when unset", () => {
    const prev = process.env.SURPRISEBOT_GATEWAY_PORT;
    process.env.SURPRISEBOT_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const surprisebot = resolveProfile(resolved, "surprisebot");
      expect(surprisebot?.cdpPort).toBe(19012);
      expect(surprisebot?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.SURPRISEBOT_GATEWAY_PORT;
      } else {
        process.env.SURPRISEBOT_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("supports explicit CDP URLs for the default profile", () => {
    ...
    const profile = resolveProfile(resolved, "surprisebot");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      controlUrl: "http://127.0.0.1:18791",
      profiles: {
        surprisebot: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("surprisebot");
  });
});

...
vi.mock("./chrome.js", () => ({
  resolveSurprisebotUserDataDir: vi.fn(() => "/tmp/surprisebot-test/surprisebot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveSurprisebotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "surprisebot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveSurprisebotUserDataDir).mockReturnValue(userDataDir);

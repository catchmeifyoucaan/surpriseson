...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".surprisebot-profile-decorated");
}
 ...
 */
export function decorateSurprisebotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SURPRISEBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SURPRISEBOT_BROWSER_COLOR).toUpperCase();

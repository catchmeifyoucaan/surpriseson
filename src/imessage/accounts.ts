...
export function listIMessageAccountIds(cfg: SurprisebotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: SurprisebotConfig): string {
...
function mergeIMessageAccountConfig(cfg: SurprisebotConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: SurprisebotConfig): ResolvedIMessageAccount[] {

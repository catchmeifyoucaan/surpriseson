...
export function resolveHooksConfig(cfg: SurprisebotConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-surprisebot-token"] === "string"
      ? req.headers["x-surprisebot-token"].trim()
      : "";

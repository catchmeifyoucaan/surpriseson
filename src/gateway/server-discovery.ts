...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Surprisebot";
  if (/surprisebot/i.test(trimmed)) return trimmed;
  return `${trimmed} (Surprisebot)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SURPRISEBOT_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "surprisebot");
  ...
  const binCli = path.join(cwd, "bin", "surprisebot.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SURPRISEBOT_TAILNET_DNS?.trim();

...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "surprisebot-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".surprisebot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<SurprisebotConfig> = {},
): SurprisebotConfig {
  const base: SurprisebotConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".surprisebot", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {

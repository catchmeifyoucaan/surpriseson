...
describe("runReplyAgent typing (heartbeat)", () => {
  it("resets corrupted Gemini sessions and deletes transcripts", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "surprisebot-session-reset-"));
  ...
  it("keeps sessions intact on other errors", async () => {
    const prevStateDir = process.env.SURPRISEBOT_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "surprisebot-session-noreset-"));
    process.env.SURPRISEBOT_STATE_DIR = stateDir;
    try {

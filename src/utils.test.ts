...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "surprisebot-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "surprisebot-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "surprisebot-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "surprisebot-lid-b-"));
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/surprisebot")).toBe(path.resolve(os.homedir(), "surprisebot"));
  });

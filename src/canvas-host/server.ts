...
<title>Surprisebot Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () => !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.surprisebotCanvasA2UIAction);
  const hasAndroid = () => !!(window.surprisebotCanvasA2UIAction && typeof window.surprisebotCanvasA2UIAction.postMessage === "function");
  const hasHelper = () => typeof window.surprisebotSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("surprisebot:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Surprisebot node canvas.");
      return;
    }
    const ok = window.surprisebotSendUserAction({
      name,
      surfaceId: "main",
      sourceComponentId,
      context: { t: Date.now() },
...
function isDisabledByEnv() {
  if (process.env.SURPRISEBOT_SKIP_CANVAS_HOST === "1") return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "surprisebot", "canvas"));
    ...
    try {
      ...
      if (!filePath) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Surprisebot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;

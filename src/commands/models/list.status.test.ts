...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSurprisebotAgentDir: vi.fn().mockReturnValue("/tmp/surprisebot-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSurprisebotAgentDir: mocks.resolveSurprisebotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/surprisebot-agent/auth-profiles.json");

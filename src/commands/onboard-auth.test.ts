...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.SURPRISEBOT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under SURPRISEBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-oauth-"));
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SURPRISEBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-minimax-"));
    process.env.SURPRISEBOT_STATE_DIR = tempStateDir;
    process.env.SURPRISEBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SURPRISEBOT_AGENT_DIR;

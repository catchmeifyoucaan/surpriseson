...
describe("onboard (non-interactive): remote gateway config", () => {
  it("writes gateway.remote url/token and callGateway uses them", async () => {
    const prev = {
      ...
      password: process.env.SURPRISEBOT_GATEWAY_PASSWORD,
    };
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-onboard-remote-"));
    ...
    try {
      ...
      process.env.SURPRISEBOT_GATEWAY_PASSWORD = prev.password;
    }
  }, 60_000);
});

...
function resolveDefaultAccountId(cfg: SurprisebotConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: SurprisebotConfig): boolean {
...
export function summarizeBindings(cfg: SurprisebotConfig, bindings: AgentBinding[]): string[] {

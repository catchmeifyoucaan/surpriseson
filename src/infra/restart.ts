...
export function triggerSurprisebotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SURPRISEBOT_SYSTEMD_UNIT,
        process.env.SURPRISEBOT_PROFILE,
      );
  ...
  const label =
    process.env.SURPRISEBOT_LAUNCHD_LABEL ||

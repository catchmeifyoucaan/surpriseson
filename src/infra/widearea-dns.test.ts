...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a surprisebot.internal zone with bridge PTR/SRV/TXT records", () => {
    const txt = renderWideAreaBridgeZoneText({
      serial: 2025121701,
      bridgePort: 18790,
      gatewayPort: 18789,
      displayName: "Mac Studio (Surprisebot)",
    ...
    expect(txt).toContain(`_surprisebot-bridge._tcp IN PTR studio-london._surprisebot-bridge._tcp`);
    expect(txt).toContain(`studio-london._surprisebot-bridge._tcp IN SRV 0 0 18790 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Surprisebot)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/surprisebot`);
  });

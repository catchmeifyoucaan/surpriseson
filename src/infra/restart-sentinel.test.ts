...
describe("restart sentinel", () => {
  ...
  beforeEach(async () => {
    prevStateDir = process.env.SURPRISEBOT_STATE_DIR;
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-sentinel-"));
    process.env.SURPRISEBOT_STATE_DIR = tempDir;
  });

  afterEach(async () => {
    if (prevStateDir) process.env.SURPRISEBOT_STATE_DIR = prevStateDir;

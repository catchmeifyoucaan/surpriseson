...
import { ensureSurprisebotCliOnPath } from "./path-env.js";

describe("ensureSurprisebotCliOnPath", () => {
  it("prepends the bundled Relay dir when a sibling surprisebot exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "surprisebot-path-"));
    try {
      const relayDir = path.join(tmp, "Relay");
      await fs.mkdir(relayDir, { recursive: true });
      const cliPath = path.join(relayDir, "surprisebot");
      await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(cliPath, 0o755);

      const originalPath = process.env.PATH;
      const originalFlag = process.env.SURPRISEBOT_PATH_BOOTSTRAPPED;
      process.env.PATH = "/usr/bin";
      delete process.env.SURPRISEBOT_PATH_BOOTSTRAPPED;
      try {
        ...
        if (originalFlag === undefined) delete process.env.SURPRISEBOT_PATH_BOOTSTRAPPED;
        else process.env.SURPRISEBOT_PATH_BOOTSTRAPPED = originalFlag;
      }
    } finally {
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const relayDir = path.join(tmp, "Relay");
      await fs.mkdir(relayDir, { recursive: true });
      const relayCli = path.join(relayDir, "surprisebot");
      await fs.writeFile(relayCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(relayCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "surprisebot");

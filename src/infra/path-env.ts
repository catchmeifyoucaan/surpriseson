...
type EnsureSurprisebotPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureSurprisebotPathOpts): string[] {
  ...
  // Bundled macOS app: `surprisebot` lives in the Relay dir (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingSurprisebot = path.join(execDir, "surprisebot");
    if (isExecutable(siblingSurprisebot)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/surprisebot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "surprisebot"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `surprisebot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSurprisebotCliOnPath(opts: EnsureSurprisebotPathOpts = {}) {
  if (process.env.SURPRISEBOT_PATH_BOOTSTRAPPED === "1") return;
  process.env.SURPRISEBOT_PATH_BOOTSTRAPPED = "1";

...
describe("browser extension install", () => {
  it("installs into the state dir (never node_modules)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "surprisebot-ext-"));
  ...
  it("copies extension path to clipboard", async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "surprisebot-ext-path-"));
    process.env.SURPRISEBOT_STATE_DIR = tmp;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.SURPRISEBOT_STATE_DIR;
      else process.env.SURPRISEBOT_STATE_DIR = prev;
    }
  });
});

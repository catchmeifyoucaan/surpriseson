  ...
  browser
    ...
    .action(async (opts: { bind?: string; port?: string; token?: string }) => {
      ...
      if (!isLoopbackBindHost(host) && !authToken) {
        defaultRuntime.error(
          danger(
            `Refusing to bind browser control on ${host} without --token (or SURPRISEBOT_BROWSER_CONTROL_TOKEN, or browser.controlToken).`,
          ),
        );
        defaultRuntime.exit(1);
      }
      ...
      defaultRuntime.log(
        info(
          [
            ...
            "",
            "Paste on the Gateway (surprisebot.json):",
            JSON.stringify(
              {
                browser: {
                  enabled: true,
            ...
            ...(authToken
              ? [
                  ...
                  `export SURPRISEBOT_BROWSER_CONTROL_TOKEN=${JSON.stringify(authToken)}`,
                ]
              : []),
          ].join("\n"),
        ),

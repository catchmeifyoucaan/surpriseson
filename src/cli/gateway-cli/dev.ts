...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = env.SURPRISEBOT_PROFILE?.trim().toLowerCase();
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - Surprisebot Dev Workspace\n\nDefault dev workspace for surprisebot gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",
...
export async function ensureDevGatewayConfig(opts: { reset?: boolean }) {
  ...
  defaultRuntime.log(`Dev config ready: ${CONFIG_PATH_SURPRISEBOT}`);

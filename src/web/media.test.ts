...
describe("web media loading", () => {
  it("compresses large local images under the provided cap", async () => {
    ...
    const file = path.join(os.tmpdir(), `surprisebot-media-${Date.now()}.jpg`);
    tmpFiles.push(file);
  ...
  it("sniffs mime before extension when loading local files", async () => {
    const pngBuffer = await sharp({
      ...
      .png()
      .toBuffer();
    const wrongExt = path.join(os.tmpdir(), `surprisebot-media-${Date.now()}.bin`);
  ...
  it("preserves GIF animation by skipping JPEG optimization", async () => {
    ...
    const file = path.join(os.tmpdir(), `surprisebot-media-${Date.now()}.gif`);
    tmpFiles.push(file);
